...
module VeniceClient
  class AudioModelPricingInput
    # USD cost per million input characters
    attr_accessor :usd

    # VCU cost per million input characters (deprecated - use Diem instead)
    attr_accessor :vcu

    # Diem cost per million input characters
    attr_accessor :diem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeniceClient::AudioModelPricingInput` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeniceClient::AudioModelPricingInput`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def usd=(usd)
      if usd.nil?
        fail ArgumentError, 'usd cannot be nil'
      end
    ...
    def diem=(diem)
      if diem.nil?
        fail ArgumentError, 'diem cannot be nil'
      end
