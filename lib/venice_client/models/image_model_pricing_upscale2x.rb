...
module VeniceClient
  class ImageModelPricingUpscale2x
    # USD cost for 2x upscale
    attr_accessor :usd

    # VCU cost for 2x upscale (deprecated - use Diem instead)
    attr_accessor :vcu

    # Diem cost for 2x upscale
    attr_accessor :diem
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `VeniceClient::ImageModelPricingUpscale2x` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `VeniceClient::ImageModelPricingUpscale2x`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
